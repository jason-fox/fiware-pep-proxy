...
const IDM = (function () {
    ...
    proxy.sendData(
      ...
      function (status, resp, headers) {
        ...
        if (response.idm_authorization_config) {
          ...
          debug(' + Authzforce enabled: ' + response.idm_authorization_config.authzforce);
          const rules =
  ...
  const checkToken = function (
    ...
    options.headers['NGSI-Entity-Id-List'] = req.locals.ids ? req.locals.ids.join(',') : '';
    options.headers['NGSI-Attribute-List'] = req.locals.attrs ? req.locals.attrs.join(',') : '';
    options.headers['NGSI-Types-List'] = req.locals.types ? req.locals.types.join(',') : '';

    const protocol = config.idm.ssl ? 'https' : 'http';

    const cachedUserInfo = checkTokenCache(token, jwtExpiration, action, resource, cache);
    if (cachedUserInfo) {
      ...
      proxy.sendData(
        ...
        function (status, resp) {
          ...
          const organizations = userInfo.organizations ? userInfo.organizations.map((elem) => elem.id) : [];

          if (!checkApplication(userInfo.app_id, userInfo.trusted_apps)) {
            debug('User not authorized in application', config.pep.app_id);
            ...
            callbackError(401, 'User not belongs to organization', organizationToken);
          } else {
  ...
  const checkTokenCache = function (token, jwtExpiration, action, resource, cache) {
    let cachedUserInfo;
    if (!cache[token]) {
      return undefined;
    }
    ...
    if (currentTime - tokenTime < config.cache_time * 1000) {
      if (config.authorization.enabled && config.authorization.pdp === 'idm') {
        if (cache[token] && cache[token][action] && cache[token][action].indexOf(resource) !== -1) {
  ...
  const checkApplication = function (appId, trusted_apps) {
