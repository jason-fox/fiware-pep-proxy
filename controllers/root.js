...
const Root = (function () {
        ...
        jsonwebtoken.verify(authToken, config.pep.token.secret, function (err, userInfo) {
          if (err) {
            if (err.name === 'TokenExpiredError') {
              ...
              checkToken(req, res, authToken, null, action, resource, authzforce, organizationToken);
            }
          } else if (config.authorization.enabled) {
            if (config.authorization.pdp === 'authzforce') {
              authorizeAzf(req, res, authToken, userInfo);
            } else if (config.authorization.pdp === 'idm') {
              checkToken(req, res, authToken, userInfo.exp, action, resource, authzforce, organizationToken);
            } else {
  ...
  const checkToken = function (req, res, authToken, jwtExpiration, action, resource, authzforce, organizationToken) {
    IDM.checkToken(
      authToken,
      jwtExpiration,
      action,
  ...
  const redirRequest =
    'auth_for_nginx' in config && config.auth_for_nginx
